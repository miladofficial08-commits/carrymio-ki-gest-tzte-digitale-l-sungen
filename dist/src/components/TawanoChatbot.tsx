...
async function sendChat(
  ...
  const data = payload as { reply?: string; leadData?: LeadData };
  if (!data.reply) throw new Error("CHAT_EMPTY_REPLY");
  return { reply: data.reply, leadData: data.leadData };
}
...
const TEASER_MESSAGES = [
  ...
  "Automatisierung — was beschäftigt Sie?",
  "Kostenloser Automations-Check?",
  "Prozesse automatisieren — sprechen Sie uns an.",
];

const GREETING_TEXT =
  "Was beschäftigt Sie gerade — Automatisierung, Chatbot oder etwas anderes?";
...
export const TawanoChatbot = () => {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const { reply, leadData } = await sendChat(chatHistory);
      ...
      // Auto-save lead when backend detected contact data
      if (leadData && (leadData.email || leadData.phone || leadData.name)) {
        saveLead({
          ...
          visitor_id: visitorId.current,
        }).catch(() => { /* silent — lead save is best-effort */ });
      }
    } catch (error: unknown) {
      const message =
